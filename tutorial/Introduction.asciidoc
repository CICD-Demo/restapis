...
To move forth with our tutorial, and implement TicketMonster, we need to clean up some of the archetype-generated code. The Member functionality, while useful for ilustrating the general setup of a Java EE 6 web application, will not be part of TicketMonster. 

Now that we are through with the introduction bits and we have generated the project structure, we can go on and delete the unneeded packages, classes, and resources.
...
Also, update the `src/main/resources/import.sql` file and remove the following line:
