...
* make changes to an existing web application for being transformed into a native application;
...
In this example, we use Apache Cordova to implement a hybrid application using the existing HTML5 mobile front-end for TicketMonster.

Tweak your application for remote access
--------------------------------------

Before we actually get to make the application hybrid, we need to make some changes in the way in which it accesses remote services. Please note that at the start of this tutorial, the changes are already implemented in the user front end, but we will show you the specific areas of code where we did just that.

In the web version of the application the client code is deployed together with the server-side code, so the models and collections (and generally any piece of code that does REST service invocations) can use URLs relative to the root of the application: all resources are serviced from the same server, so the browser will do the correct invocation. This is also consistent with the same origin policy, to prevent cross-site scripting attacks.

Once the client code is deployed separately from the services, the REST invocations must use absolute URLs (we will see the same-origin policy implications and how to solve that a bit further). Furthermore, since the location of the application deployment can change during the lifetime of the application, it must be configurable.

You already have caught a glimpse of this in the user front end chapter. We allowed for it by defining the `configuration` module for the mobile version of the application. 
...
This module has a `baseURL` property that is either set to an empty string for relative URLs or to an actual prefix value, depending on whether a global variabile named `TicketMonster` has already been defined, and it has a `baseRESTUrl`
property.
...
The prefix is applied in a similar fashion to all the other modules that perform REST service invocations. You don't need to do anything additional right now, because the code we created in the user front end tutorial is already defined like this. However, as said earlier, for mobile web applications that use relative URLs exclusively, introducing some form of URL customization is a requirement for being able to migrate them in a hybrid form. 
...
Creating an Android project using Phonegap
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Creating an iOS project using Phonegap
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Now we create a symbolic link inside the 'ios' directory to the original TicketMonster project, with the name `www`.
    ...
    $ ln -s www $TICKET_MONSTER_HOME/

Now we can open the created project in Xcode. 

Just as in the case of the Android application, we will customize the user agent information that gets passed on to the browser. We will use this information to load the proper JavaScript library. So we will adjust the `initialize` method in the generated code to that effect.
