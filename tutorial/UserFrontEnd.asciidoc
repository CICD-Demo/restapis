...
As you can see, the page does not contain much. It detects the current client (mobile or desktop) based on its capabilities (touch or not) and loads the JavaScript code of the page from an AMD module defined in the `resources/js/configurations` folder using `require.js`. In the case of the desktop client, the code is loaded from `resources/js/configurations/desktop.js`.
