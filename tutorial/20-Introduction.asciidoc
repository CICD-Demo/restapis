...
TicketMonster is developed as an open source application, and you can find it under link:https://github.com/jboss-jdf/ticket-monster[at github]. 
...
In this tutorial, we will cover the following topix:
...
You must have a Java Development Kit (JDK) installed, either v6 or v7 will work - whils a JVM runtime will work for most use cases, for a developer environment it is normally best to have the full JDK. System requirements for JBoss Developer Studio are link:http://docs.redhat.com/docs/en-US/JBoss_Developer_Studio/5.0.Beta/html/Getting_Started_Guide/minimumrequirements.html
...
You can skip that step in the installation wizard that allows you to install JBoss Enterprise Application Platform 6 or JBoss AS 7 as we will do this in the next step.
...
You will be prompted with a dialog box that verifies that JBoss Devloper Studio is configured correctly. If you are in a brand new workspace, the application server will not be configured yet and you will notice the lack of a check mark on the server/runtime row.
...
The generated project is a Maven-based project with a `pom.xml` in its root directory and the standard Maven project layout.
...
`.model`::
    contains the JPA entity class, a POJO annotated annotated with `@Entity`, annodated with Bean Validation (JSR 303) constraints
`.rest`::
    contains the JAX-RS endpoints, POJOs annoated with `@Path`
`.service`::
...
`main/resources/META-INF/persistence.xml`::
    establishes that this project contains JPA entities and it identifies the datasource, which is deployed alongside the project.  

    It also includes the `hibernate.hbm2dll.auto` property set to `create-drop` by default. 
`test/java/test`::
    provides the `.test` package that contains `MemberRegistrationTest.java`, an Arquillian based test that runs both at the command line 
        mvn test –Parq-jbossas-remote
   
    as well as from within JBoss Developer Studio *Run As -> JUnit Test*.
`src/main/webapp`::
...
Forge is a command-oriented rapid application development tool that allows you to enter commands that generate classes and code. It will automatically update the IDE for you.  A key features is "content assist" or "tab completion", activated by pressing *tab*.
...
Forge has automatically changed the context of the CLI to `Event.java, and typing `ls` – this will provide a listing of the fields and methods.  
...
You should end up the command `field string --named name`, to execute it, press enter.  This will add a `private String name;` field, and the appropriate accessor and mutator (getter and setter) methods.  You should also notice that the toString method is tweaked to include `name` as well.
...
By default, the entity classes generate the database schema, and is controoled by `src/main/resources/persistence.xml`.
...
The UI components interact with the JAX-RS RESTful services (e.g. `EventService.java`) and we refer to this architecture as POH5 – for plain old HTML5 – to help distringuish it from JSF-based or GWT-based architectures. In a POH5 application, all the rendering of views is done in the client, with the server just providing data.

[TIP]
=================================================================================
For more information on building POH5 applications with JBoss technologies, check
